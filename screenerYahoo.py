...
def getAllYahoo(forex_api, basedir, _yahooList) -> Tuple[pl.DataFrame, list]:
    ...
    for filename in _yahooList:
        ...
        for labelStr, nameStr in assetList.items():
            ...
            row['L%H'] = 100.0*(row['__nprice'] - row['NLOW']) / (row['NHIG'] - row['NLOW']) if (row['NHIG'] - row['NLOW']) != 0 else 0
            if 'PEINCLXOR' not in row:
            ...
            df = pl.DataFrame(ohlc).rename({"Close": "close", "Volume": "volume", "Open": "open"})
            # display(df)
